# SisOp-2025
# LATIHAN

## 1. Basis Bilangan

### A. Bilangan Biner
- **Soal**: Bilangan Biner Adalah Bilangan Yang Berbasis ..
- **Jawaban**: dua.

### B. Bilangan Heksadesimal
- **Soal**: Bilangan Heksadesimal Adalah Bilangan Yang Berbasis..
- **Jawaban**: Enam Belas.

---

## 2. Konversi Bilangan Desimal ke Biner

### A. 1234₁₀ = 10011010010₂
| Pembagian | Hasil Bagi | Sisa |
|-----------|------------|------|
| 1234 : 2  | 617        | 0    |
| 617 : 2   | 308        | 1    |
| 308 : 2   | 154        | 0    |
| 154 : 2   | 77         | 0    |
| 77 : 2    | 38         | 1    |
| 38 : 2    | 19         | 0    |
| 19 : 2    | 9          | 1    |
| 9 : 2     | 4          | 1    |
| 4 : 2     | 2          | 0    |
| 2 : 2     | 1          | 0    |
| 1 : 2     | 0          | 1    |

### B. 5670₁₀ = 111111100110₂

### C. 2321₁₀ = 100100010001₂
---

## 3. Konversi Bilangan Biner ke Desimal

### A. 10101010₂ = 170₁₀
| Bit | Pangkat | Hasil |
|-----|---------|-------|
| 0   | 2⁰      | 0     |
| 1   | 2¹      | 2     |
| 0   | 2²      | 0     |
| 1   | 2³      | 8     |
| 0   | 2⁴      | 0     |
| 1   | 2⁵      | 32    |
| 0   | 2⁶      | 0     |
| 1   | 2⁷      | 128   |
| **Total** |         | **170** |

### B. 01010101₂ = 85₁₀
### C. 11001100₂ = 204₁₀

### D. 10011111₂ = 159₁₀
---

## 4. Konversi Bilangan Biner ke Oktal

### A. 101011111001₂ = 5371₈
| Grup Biner | Oktal |
|------------|-------|
| 001        | 1     |
| 111        | 7     |
| 011        | 3     |
| 101        | 5     |

### B. 110010110111₂ = 6267₈
---

## 5. Konversi Bilangan Oktal ke Biner

### A. 2170₈ = 010001111000₂
| Oktal | Biner |
|-------|-------|
| 0     | 000   |
| 7     | 111   |
| 1     | 001   |
| 2     | 010   |

### B. 3571₈ = 011101111001₂

---

## 6. Konversi Bilangan Desimal ke Heksadesimal

### A. 1780₁₀ = 06F4
| Pembagian | Hasil Bagi | Sisa | Heksadesimal |
|-----------|------------|------|--------------|
| 1780 : 16 | 111        | 4    | 4            |
| 111 : 16  | 6          | 15   | F            |
| 6 : 16    | 0          | 6    | 6            |

### B. 3666₁₀ = 0E52
### C. 5230₁₀ = 146E
### D. 6744₁₀ = 1A58

---

## 7. Konversi Bilangan Pecahan Heksadesimal ke Biner

### A. ABCD₁₆ = 43981
| Digit | Pangkat | Hasil |
|-------|---------|-------|
| D     | 16⁰     | 13    |
| C     | 16¹     | 192   |
| B     | 16²     | 2816  |
| A     | 16³     | 40960 |

### B. 2170₁₆ = 7560
### C. B75F₁₆ = 46943
### D. EBED₁₆ = 69397

---

## 8. Konversi Bilangan Pecahan Desimal ke Biner

### A. 0,3125₁₀ = 0,0101₂
| Langkah | Perkalian | Hasil | Bit |
|---------|-----------|-------|-----|
| 1       | 0,3125 x 2 | 0,625 | 0   |
| 2       | 0,625 x 2  | 1,25  | 1   |
| 3       | 0,25 x 2   | 0,5   | 0   |
| 4       | 0,5 x 2    | 1,0   | 1   |

### B. 0,65625₁₀ = 0,10101₂

### C. 0,34375₁₀ = 0,01011₂
### D. 0,140625₁₀ = 0,001001₂

---

## 9. Konversi Bilangan Desimal ke Biner

### A. 11,625₁₀ = 1011,101₂
### B. 0,6875₁₀ = 0,1011₂

### C. 0,75₁₀ = 0,11₂
| Langkah | Perkalian | Hasil | Bit |
|---------|-----------|-------|-----|
| 1       | 0,75 x 2  | 1,5   | 1   |
| 2       | 0,5 x 2   | 1,0   | 1   |

### D. 25,75₁₀ = 11011,11₂
---
## 10. Konversi Bilangan Desimal ke Heksadesimal

### A. 348,654₁₀ = 15C,A76
#### Bilangan Integer 348
| Pembagian | Hasil Bagi | Sisa | Heksadesimal |
|-----------|------------|------|--------------|
| 348 : 16  | 21         | 12   | C            |
| 21 : 16   | 1          | 5    | 5            |
| 1 : 16    | 0          | 1    | 1            |

#### Bilangan Pecahan 0,654
| Langkah | Perkalian | Hasil | Heksadesimal |
|---------|-----------|-------|--------------|
| 1       | 0,654 x 16 | 10,464 | A            |
| 2       | 0,464 x 16 | 7,424  | 7            |
| 3       | 0,424 x 16 | 6,784  | 6            |

### B. 1784,240₁₀ = 6F8,3D7
---

## 11. Konversi Bilangan ke Desimal

### A. 010100011,001111101₂ = 163,245₁₀
#### Bilangan Integer 010100011
| Bit | Pangkat | Hasil |
|-----|---------|-------|
| 1   | 2⁰      | 1     |
| 1   | 2¹      | 2     |
| 0   | 2²      | 0     |
| 0   | 2³      | 0     |
| 0   | 2⁴      | 0     |
| 1   | 2⁵      | 32    |
| 0   | 2⁶      | 0     |
| 1   | 2⁷      | 128   |
| 0   | 2⁸      | 0     |
| **Total** |         | **163** |

#### Bilangan Pecahan 001111101
| Bit | Pangkat | Hasil       |
|-----|---------|-------------|
| 0   | 2⁻¹     | 0           |
| 0   | 2⁻²     | 0           |
| 1   | 2⁻³     | 0,125       |
| 1   | 2⁻⁴     | 0,0625      |
| 1   | 2⁻⁵     | 0,03125     |
| 1   | 2⁻⁶     | 0,015625    |
| 1   | 2⁻⁷     | 0,0078125   |
| 0   | 2⁻⁸     | 0           |
| 1   | 2⁻⁹     | 0,001953125 |
| **Total** |         | **0,244140625** |

### B. 654,276₈ = 428,37109375₁₀

### C. 4C5,2BB₁₆ = 1221,169921875₁₀
---

## 12. Konversi Bilangan Biner ke BCD

### A. 10100110000111₂ = 2987
| Grup Biner | BCD |
|------------|-----|
| 0111       | 7   |
| 1000       | 8   |
| 1001       | 9   |
| 0010       | 2   |

### B. 1010101100011₂ = 1563

---

## 13. Konversi BCD ke Bilangan Biner

### A. 1987 = 1 1001 1000 0111
| Digit | Biner |
|-------|-------|
| 1     | 0001  |
| 9     | 1001  |
| 8     | 1000  |
| 7     | 0111  |

### B. 2346 = 10 0011 0100 0110

### C. 501 = 101 0000 0001

---

## 14. Konversi Bilangan Biner ke BCO

### A. 11111101001₂ = 3751
| Grup Biner | Oktal |
|------------|-------|
| 001        | 1     |
| 101        | 5     |
| 111        | 7     |
| 011        | 3     |

### B. 101110010100₂ = 5624

### C. 1100000010₂ = 1402
---

## 15. Konversi Bilangan Biner ke BCH

### A. 1101111100101110₂ = CF2E
| Grup Biner | Heksadesimal |
|------------|--------------|
| 1110       | E            |
| 0010       | 2            |
| 1111       | F            |
| 1101       | C            |

### B. 110100110000001₂ = 6981
---

## 16. Konversi BCH ke Bilangan Heksadesimal

### A. F0DE = 1111 0000 1101 1110
| Digit | Biner |
|-------|-------|
| F     | 1111  |
| 0     | 0000  |
| D     | 1101  |
| E     | 1110  |

### B. 1CAB = 0001 1100 1010 1011

### C. 834 = 1000 0011 0100
---

## 17. Bilangan Biner Positif atau Negatif

### A. 01111111 → **Positif**
### B. 10000000 → **Negatif**
### C. 01111011 → **Positif**

---

## 18. Bilangan Biner Negatif ke Desimal

### A. 10001000 → -128 + 8 = **-120**
### B. 11110111 → -128 + 1 + 2 + 4 + 16 + 32 + 64 = **-9**
### C. 10000101 → -128 + 1 + 4 = **-123**
### D. 10011100 → -128 + 4 + 8 + 16 = **-100**

---

## 19. ASCII Code ke Karakter

### A. 4116 → 4 x 16 + 1 = 65 → **A**
### B. 5A16 → 5 x 16 + 10 = 90 → **Z**
### C. 2416 → 2 x 16 + 4 = 36 → **$**
### D. 7716 → 7 x 16 + 7 = 119 → **w**

---

## 20. Karakter ke ASCII Code

### A. a → 97₁₀ = 61₁₆
### B. x → 120₁₀ = 78₁₆
### C. m → 108₁₀ = 6D₁₆
### D. h → 72₁₀ = 48₁₆

---

## 21. Keluaran Keyboard ASCII

### PRINT X = 50 52 49 4E 54 20 58
| Karakter | ASCII (Hex) |
|----------|-------------|
| P        | 50          |
| R        | 52          |
| I        | 49          |
| N        | 4E          |
| T        | 54          |
| Spasi    | 20          |
| X        | 58          |
